...
MongoDB Change Streams non funzionano su istanze standalone. Un servizio MongoDB self-hosted via Coolify (one-click service) non ha di default il replica set abilitato — usare MongoDB Atlas (anche M0 free supporta Change Streams). Copiare la connection string Atlas come variabile `MONGODB_URI` nel servizio backend su Coolify.
